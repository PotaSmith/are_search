...
RSpec.describe "search paging" do
    ...
    it "SingleSearch は max_result_window を超える最後のページの size を縮める" do
        body = AreSearch::SingleSearch.search(
            article_index_target,
            AreSearch::DumpBody,
            fields:   [:title],
            page:     2,
    ...
    it "DB 復元で落ちた hit の件数を total_count から差し引き、ES の件数は es_total_count に残す" do
        ...
        response = {
            "hits" => {
                "hits"  => [
                    { "_index" => "test_articles_default", "_id" => "1" },
                    { "_index" => "test_articles_default", "_id" => "2" },
                ],
                "total" => { "value" => 100 },
            },
        }
