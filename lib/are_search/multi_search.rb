...
module AreSearch
    module MultiSearch
        ...
        def search(index_targets, query, **options)
            ...
            require_fields!(fields_opts, caller_name: :multi_search)
            ...
            # --- バリデーション ---
            validate_combined_fields!(ctx, normalized_fields, index_targets, valid_fields, caller_name: :multi_search)
            validate_sort!(ctx, sort_opts, valid_fields, caller_name: :multi_search)
            validate_where!(ctx, where_opts, valid_fields, caller_name: :multi_search)
            validate_where_not!(ctx, where_not_opts, valid_fields, caller_name: :multi_search)
            validate_should!(ctx, should_opts, minimum_should_match_opts, valid_fields, caller_name: :multi_search)
            validate_aggs!(ctx, aggs_opts, valid_fields, caller_name: :multi_search)
            validate_highlight!(ctx, highlight_opts, normalized_fields.map { |f| f[:name] }, valid_fields, caller_name: :multi_search)
            # results_whereとincludesはarに直接渡す前提のため無加工であること。to_symもしない
            validate_results_where!(ctx, model_results_where_opts, models, caller_name: :multi_search)
            validate_includes!(ctx, model_includes_opts, models, caller_name: :multi_search)
            ...
            if ctx[:highlight_fields].any?
                body[:highlight] =
                    build_highlight_body(ctx[:highlight_fields], highlight_opts)
            end
